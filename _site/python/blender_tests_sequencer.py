...
def put_images_on_sequencer(images, folder, secs):
    ...
    fps=24
    for i in range(len(images)):
        stop=fps*secs[i] + start 
        bpy.ops.sequencer.image_strip_add(
            directory=folder, files=[{"name":'{}.jpg'.format(images[i])}], 
            relative_path=True, show_multiview=False, frame_start=start, frame_end=stop, channel=int(i%10))
        start = stop + 1
...
bpy.ops.script.python_file_run(filepath="/usr/share/blender/2.77/scripts/presets/render/HDTV_720p.py")
...
bpy.context.scene.render.filepath = "/tmp/final6.avi"
